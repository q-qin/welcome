  ...
  <div>
    ...
    <div class="taskCenter" v-show="!loading">
      <div class="taskCenter" v-show="!loading">
        ...
        <!-- 活动广告位 -->
        <div class="ad"  v-show="!iospedding">
          <!-- 第三方广告位 -->
          <div id="hancr_002" class="baidu" v-if="thirdArr.length!=0">
          </div>
        ...
        <div class="v-modal" v-show="yindaoShow&&!iospedding" @touchmove.prevent>
          <div class="yindao modal">
            <img :src="require('@/assets/images/taskcenter/yindao.jpg')">
...
// let comm = {
//     uid: '11009067',
//     token: 'aa8fcc8a515854229fbc7122534c627f',
...
export default { 
  ...
  methods: {
      ...
      if (this.isLogin) {
        ...
        if (this.detail.is_sign != 1) {
          ...
          if (rs && rs.code === 1001) {
            ...
            //原生签到弹窗
            appcall.nativeSign({coin:rs.data.today_coin});
          } else if (rs.code === 1005) {
            this.$alert(rs.msg).then(() => {
              appcall.login();
            });
        ...
        if (!this.flag) {
          ...
          if (
            ...
            if (item.task_count == 0) {
              ...
              appcall.navigator({
                title: "兑换提现",
                url: this.$v3url + '/main/user.html#/withdrawal'
              });
            }
    ...
    jumpRules(position) {
      if (!!position) {
        _hmt.push(["_trackEvent", "taskcenter", "click", position]);
      }
      appcall.navigator({
        title: "任务规则说明",
        url: this.$v3url + '/subject/taskcenter.html#/rule'
      });
    },
    /**
     * 微信朋友圈邀请
    ...
    thirdAd(data) {
      data.forEach(item => {
        switch (item.ad_source) {
          case "baidu":
            setTimeout(()=>{
              BAIDU_CLB_fillSlotAsync(item.slot_id, "hancr_002");
            },1000)
            break;
          case "xunfei":
            break;
        ...
        a {
          ...
          .taskContent {
            ...
            .taskInfo {
              ...
              .hot {
                height: 0.21rem;
                .background('taskcenter/v120_hot.png');
