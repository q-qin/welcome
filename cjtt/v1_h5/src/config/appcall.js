var appcall = {
  ...
  getRequestData: function (requestParams) {
    return new Promise((resolve, reject) => {
      ...
      self.params = {};
      try {
        self.getAppData().then(appdata => {
          // app参数 => params
          delete appdata.reviewstatus; // 删除reviewstatus属性
          Object.assign(self.params, appdata, self.params);
          // app参数+业务参数 => sign
          let signParams = Object.assign({}, appdata, requestParams);
          self.getSign(signParams).then(sign => {
            // params+sign => params;
            Object.assign(self.params, sign, self.params);
            resolve(self.params);
          })
        })
      } catch (e) {
  ...
  shareArticle: function (params,callback) {
    JKEventHandler.callNativeFunction('shareArticle', params, '')
  },
  // 打开某条特定资讯 参数：资讯id
